-- Re-apply the approved administrator roster for databases that already ran
-- the initial schema migration but did not receive the roster rows.
-- The application keeps one canonical full-access role: the roster's
-- "Master Admin" label is stored as `administrator`.
...
ON CONFLICT(email) DO UPDATE SET
  ...
  invited_by = CASE WHEN trim(coalesce(admin_users.invited_by, '')) = '' THEN excluded.invited_by ELSE admin_users.invited_by END,
  invited_at = COALESCE(admin_users.invited_at, excluded.invited_at),
  activated_at = CASE WHEN admin_users.status = 'active' THEN COALESCE(admin_users.activated_at, excluded.activated_at) ELSE admin_users.activated_at END,
...
SELECT
  'audit-roster-sync-' || lower(replace(replace(email, '@', '-at-'), '.', '-')),
  NULL,
  'roster-import',
